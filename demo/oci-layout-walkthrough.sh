...
cargo run -q -- validate "$catalog_json"
...
cargo run -q -- oci export-layout --tag walkthrough --cosign-key "$cosign_key" --cosign-public-key "$cosign_pub" "$catalog_json" "$layout_dir"
...
cargo run -q -- oci unpack-layout --ref-name walkthrough "$layout_dir" > "$roundtrip_json"
cat "$roundtrip_json"
...
cargo run -q -- validate "$roundtrip_json"

step "Push the catalog into a second OCI layout with ORAS mediation and Cosign artifacts"
cargo run -q -- oci push --cosign-key "$cosign_key" --cosign-public-key "$cosign_pub" "$catalog_json" "$target_ref" --to-oci-layout-path "$copied_layout_dir"
